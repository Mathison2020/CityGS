...
def block_partitioning(cameras, gaussians, args, pp, scale=1.0, quiet=False, disable_inblock=False, simple_selection=False):
        ...
        if args.aabb is None:
            ...
            if not os.path.exists(os.path.join(args.source_path, "data_partitions")):
                os.makedirs(os.path.join(args.source_path, "data_partitions"))
            ...
            Min_x, Min_y, Min_z = args.aabb[:3]
            Max_x, Max_y, Max_z = args.aabb[3:]

            step_x = (Max_x - Min_x) / args.block_dim[0]
            step_y = (Max_y - Min_y) / args.block_dim[1]
            step_z = (Max_z - Min_z) / args.block_dim[2]
        ...
        with torch.no_grad():
            ...
            for block_id in range(block_num):
                ...
                # min_x, max_x = Min_x + block_id_x * step_x, Min_x + (block_id_x + 1) * step_x
                # min_y, max_y = Min_y + block_id_y * step_y, Min_y + (block_id_y + 1) * step_y
                # min_z, max_z = Min_z + block_id_z * step_z, Min_z + (block_id_z + 1) * step_z
